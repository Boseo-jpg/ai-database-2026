...
def get_students():
    # 실제 DB 연결
    con = get_connection()  # 연결 확인하려면 F12
    cursor = con.cursor(row_factory=dict_row)   # 마우스커서처럼 테이블 한행씩 가리키는 값

    try:
        # 실제 쿼리는 DBeaver 등에서 작성 확인 후 복사해옴
        cursor.execute("""
    ...
    # except Exception:         # 예외
        ...
        con.close() # 예외가 발생하든 안하든 무조건 DB연결은 닫아야함
...
def create_student(student: StudentModel):
    ...
    try:
        ...
        return {'message' : 'Student rdgistration done'}
